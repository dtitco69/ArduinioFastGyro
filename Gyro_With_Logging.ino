...
{      
  ...
  for (byte i = 0; i < 100; i++) {
    ...
    if (SD.exists(FileName)) {
      ...
      dataFile.println("X_ACCEL,Y_ACCEL,Z_ACCEL,X_GYRO,Y_GYRO,Z_GYRO,TEMP(C),TIME(ms)"); //when file found set headers
      dataFile.close();                                     // Close the File
      Serial.println("");
      Serial.println("Setting Headers");
      Serial.println("X_ACCEL,Y_ACCEL,Z_ACCEL,X_GYRO,Y_GYRO,Z_GYRO,TEMP(C),TIME(ms)");
      Serial.println("");
      break;
  }
...
void loop()
{
  ...
  String dataString = "";                                   // reset the data string to blank
    ...
    dataString += accel_t_gyro.value.x_accel, DEC;  // append the sensor value to the datastring
    dataString += ",";                                      // append a comma to the string
    dataString += accel_t_gyro.value.y_accel, DEC; 
    dataString += ",";                                     
    dataString += accel_t_gyro.value.z_accel, DEC; 
    dataString += ",";
    dataString += accel_t_gyro.value.x_gyro, DEC; 
    dataString += ",";   
    dataString += accel_t_gyro.value.y_gyro, DEC; 
    dataString += ",";   
    dataString += accel_t_gyro.value.z_gyro, DEC; 
    dataString += ","; 
      
    dT = ( (double) accel_t_gyro.value.temperature + 12412.0) / 340.0;    //Calculate temperature
//       Serial.println(dT);
    char TestString[5];
    dtostrf(dT,5,2,TestString);
    dataString += TestString;
    
    //dataString += String(dT);                           // Append Temp to string
    dataString += ",";
    dataString += millis();                                // Append time to string
